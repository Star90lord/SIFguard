...
async function fetchReports(filter = {}, select = "") {
  if (mongoose.connection && mongoose.connection.readyState === 1) {
    try {
      const q = Report.find(filter);
      if (select) q.select(select);
      return await q.lean();
    } catch (err) {
      console.warn("MongoDB Report query error in analyticsEngine:", err.message);
    }
  }
  return [];
}
...
async function getOverallStatistics(filter = {}) {
  const reports = await fetchReports(
    filter,
    "entities sif_precursor processingStatus createdAt"
  );
...
async function getHazardPatterns(filter = {}, limit = 10) {
  const reports = await fetchReports(filter, "entities.hazards");
...
async function getActivityPatterns(filter = {}, limit = 10) {
  const reports = await fetchReports(filter, "entities.activities");
...
async function getLocationPatterns(filter = {}, limit = 10) {
  const reports = await fetchReports(filter, "entities.locations");
...
async function getBarrierFailurePatterns(filter = {}, limit = 10) {
  const reports = await fetchReports(filter, "entities.barrierFailures");
...
async function getHazardSeverityAnalysis(filter = {}) {
  const reports = await fetchReports(
    filter,
    "entities.hazards sif_precursor.severity"
  );
...
async function getEmergingTrends(filter = {}, days = 30) {
  ...
  const previousReports = await fetchReports(
    {
      ...filter,
      createdAt: {
        $gte: previousStart,
