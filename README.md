...
dataset/
│
├── sif_ner_dataset (1).jsonl   <- annotated NER dataset (character spans)
├── sifguard_train.csv          <- tabular SIF-precursor reference data
├── sifguard_sif_precursor_TEST.csv
│
...
```powershell
pip install torch transformers accelerate python-docx
```

> No `datasets` / `evaluate` / `seqeval` / `scikit-learn` packages are
> needed: the training pipeline uses plain JSON + `torch.utils.data` and a
> dependency-free entity-level metric module
> (`nlp_service/training/ner_metrics.py`). Do NOT create a top-level
> `datasets/` directory — it shadows the Hugging Face `datasets` package.
...
```powershell
SIFvenv\Scripts\python.exe -m uvicorn nlp_service.main:app --host 127.0.0.1 --port 8000
```
...
The annotated NER dataset lives at:

```text
dataset/sif_ner_dataset (1).jsonl
```

Validate it first (pure standard library, no torch required):

```powershell
SIFvenv\Scripts\python.exe nlp_service/training/validate_dataset.py
```

Then prepare token-level splits:
...
Run (from the project root, so `dataset/` and `nlp_service/` resolve):
...
alongside `training_config.json` and `labels.json`.
