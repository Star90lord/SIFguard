      ...
      <PageContainer className="space-y-6">
        ...
        <div className="p-6 rounded-xl border border-slate-200 bg-white shadow-2xs space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            ...
            {/* Selected Facilities Chips & Add Facility Dropdown (Unlimited) */}
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/* Add Site Dropdown for Unlimited Facility Comparison */}
              ...
              {/* Compare All Button if some sites not yet selected */}
              {availableToAdd.length > 0 && (
                <Button
                  variant="secondary"
                  size="sm"
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-1 border-b border-slate-200">
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2 text-xs font-semibold border-b-2 transition-all ${
                  activeTab === 'overview'
                    ? 'border-blue-600 text-blue-700 bg-blue-50/40 rounded-t'
              ...
              >
                ...
                <span className="px-1.5 py-0.2 text-[10px] rounded bg-slate-100 text-slate-600 font-mono font-normal">
                  Focused
                </span>
              </button>
            ...
            <div className="text-[11px] text-slate-500 font-mono flex items-center gap-2">
              <span>Evaluating {comparedSites.length} facilities</span>
              <span>·</span>
              <span>{summary ? summary.totalReports : 0} total observations</span>
            </div>
          </div>
        </div>
        ...
        {/* LOADING STATE */}
        {loading ? (
          <div className="p-16 text-center bg-white border border-slate-200 rounded-xl shadow-2xs space-y-3">
            <div className="w-8 h-8 border-3 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto" />
            <div>
              <p className="text-sm font-bold text-slate-900">Building cross-site safety comparison...</p>
          ...
          /* ERROR STATE */
          <div className="p-8 text-center bg-white border border-rose-200 rounded-xl shadow-2xs space-y-3">
            <div className="w-10 h-10 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center mx-auto">
              <AlertOctagon size={20} />
            </div>
            <div>
              <h3 className="text-sm font-bold text-slate-900">Unable to build comparison</h3>
              <p className="text-xs text-slate-500 mt-1">{error}</p>
            </div>
            <Button variant="primary" size="sm" icon={RefreshCw} onClick={loadComparison}>
              Retry Comparison
            </Button>
          </div>
        ) : comparedSites.length === 0 || (summary && summary.totalReports === 0) ? (
          /* CASE 8: EMPTY STATE (NO MATCHING REPORTS) */
          <div className="p-12 text-center bg-white border border-slate-200 rounded-xl shadow-2xs space-y-4 max-w-md mx-auto">
            <div className="w-12 h-12 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center mx-auto">
              <FileText size={24} />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900">No report data available</h3>
              <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
          ...
          <>
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                {/* 1. COMPARISON SUMMARY (Section 3.A) */}
                {summary && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                    ...
                    <div className="p-3.5 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-1">
                      ...
                      <div className="flex items-baseline gap-1.5">
                        ...
                        <span className="text-[11px] text-slate-500 font-medium">operational</span>
                      </div>
                    ...
                    <div className="p-3.5 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-1">
                      ...
                      <p className="text-[11px] text-slate-500">Across selected scope</p>
                    </div>
                    ...
                    <div className="p-3.5 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-1">
                      ...
                      <div className="flex items-baseline gap-1.5">
                        <span className={`text-2xl font-bold font-mono ${summary.highRiskReports > 0 ? 'text-orange-600' : 'text-slate-400'}`}>
                          {summary.highRiskReports}
                        </span>
                        <span className="text-[11px] text-slate-500">observations</span>
                      </div>
                      <p className="text-[11px] text-slate-500">Elevated severity</p>
                    </div>
                    ...
                    <div className="p-3.5 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-1">
                      ...
                      <div className="flex items-baseline gap-1.5">
                        <span className={`text-2xl font-bold font-mono ${summary.sifPrecursors > 0 ? 'text-red-600' : 'text-slate-400'}`}>
                          {summary.sifPrecursors}
                        </span>
                        <span className="text-[11px] text-red-600 font-medium">signals</span>
                      </div>
                      <p className="text-[11px] text-slate-500">Acute barrier risk</p>
                    </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                          <th className="py-3 px-4 min-w-[200px] sticky left-0 z-20 bg-slate-50 border-r border-slate-200/80 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.06)]">
                            Facility
                          </th>
                          <th className="py-3 px-3 text-right font-semibold">Total Reports</th>
                          <th className="py-3 px-3 text-right font-semibold text-emerald-700">LOW</th>
                          <th className="py-3 px-3 text-right font-semibold text-amber-700">MEDIUM</th>
                          <th className="py-3 px-3 text-right font-semibold text-orange-700">HIGH</th>
                          <th className="py-3 px-3 text-right font-semibold text-rose-700">SIF-PRECURSOR</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {comparedSites.map((site) => (
                          <tr key={site.id} className="hover:bg-slate-50/60 transition-colors">
                            {/* Site (Sticky) */}
                            <td className="py-3.5 px-4 sticky left-0 z-10 bg-white border-r border-slate-200/80 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.06)]">
                              <div className="flex items-center gap-2">
                            ...
                            {/* High Risk */}
                            <td className="py-3.5 px-3 text-right font-mono font-bold text-orange-700">
                              {site.highRiskCount}
                            </td>
                            ...
                            <td className="py-3.5 px-4 text-slate-800">
                              {site.topHazard && site.topHazard.count > 0 ? (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-slate-100 text-slate-700 text-[11px] font-medium">
                                  ...
                                  <span className="font-mono font-bold text-slate-900">({site.topHazard.count})</span>
                                </span>
                              ) : (
                            ...
                            <td className="py-3.5 px-4 text-slate-800">
                              {site.primaryBarrier && site.primaryBarrier.count > 0 && site.primaryBarrier.barrier !== 'None Recorded' ? (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-rose-50 text-rose-800 border border-rose-200/60 text-[11px] font-medium">
                                  {site.primaryBarrier.barrier}
                                  <span className="font-mono font-bold text-rose-900">({site.primaryBarrier.count})</span>
                                </span>
                              ) : (
                ...
                {/* 3. SIDE-BY-SIDE METRIC BENCHMARK TABLE (STICKY METRIC COLUMN FOR UNLIMITED SITES) */}
                <div className="bg-white border border-slate-200 rounded-xl shadow-2xs overflow-hidden">
                  ...
                  <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-200">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                          ...
                          {comparedSites.map((site) => (
                            <th
                              key={site.id}
                              className="py-3 px-4 min-w-[175px] font-bold text-slate-900 border-l border-slate-100"
                            >
                              <Link
                                to={`/sites/${site.id}`}
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {/* Health Status */}
                        <tr>
                          <td className="py-3 px-4 font-semibold text-slate-700 w-48 min-w-[190px] sticky left-0 z-10 bg-white border-r border-slate-200/80 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.06)]">
                            Health Status
                          </td>
                          {comparedSites.map((s) => (
                            <td key={s.id} className="py-3 px-4 min-w-[175px] border-l border-slate-100">
                        ...
                        <tr>
                          ...
                          {comparedSites.map((s) => (
                            <td key={s.id} className="py-3 px-4 min-w-[175px] font-mono font-bold text-slate-900 text-sm border-l border-slate-100">
                              {s.totalReports}
                            </td>
                          ))}
                        </tr>
                        ...
                        <tr>
                          ...
                          {comparedSites.map((s) => (
                            <td key={s.id} className="py-3 px-4 min-w-[175px] text-[11px] border-l border-slate-100">
                              {s.topHazard?.hazard ? (
                                <span className="font-semibold text-slate-800">
                                  {s.topHazard.hazard}{' '}
                                  <span className="font-mono text-slate-500 font-normal">({s.topHazard.count})</span>
                                </span>
                              ) : (
                            ...
                            <td key={s.id} className="py-3 px-4 min-w-[175px] text-[11px] border-l border-slate-100">
                              {s.topActivity?.activity ? (
                                <span className="text-slate-700">
                                  {s.topActivity.activity}{' '}
                                  <span className="font-mono text-slate-400">({s.topActivity.count})</span>
                                </span>
                              ) : (
                                <span className="text-slate-400 italic">—</span>
                              )}
                            ...
                            <td key={s.id} className="py-3 px-4 min-w-[175px] text-[11px] border-l border-slate-100">
                              {s.primaryBarrier?.barrier && s.primaryBarrier.barrier !== 'None Recorded' ? (
                                <span className="text-rose-700 font-medium">
                                  ...
                                  <span className="font-mono font-bold text-rose-900">({s.primaryBarrier.count})</span>
                                </span>
                              ) : (
                                <span className="text-slate-400 italic">Zero</span>
                              )}
                            </td>
                          ))}
                        </tr>
                        ...
                        <tr>
                          ...
                          {comparedSites.map((s) => (
                            <td key={s.id} className="py-3 px-4 min-w-[175px] font-mono text-slate-500 border-l border-slate-100">
                              {s.lastActivity}
                            </td>
                          ))}
                        </tr>
                ...
                {/* 4. SAFETY INTERPRETATION: CONCISE KEY FINDINGS (Section 3.D) */}
                <div className="p-6 rounded-xl border border-slate-200 bg-white shadow-2xs space-y-4">
                    ...
                    {keyFindings.map((kf) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] font-mono text-slate-400">{kf.siteCode}</span>
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {comparedSites.map((site) => (
                      <div key={site.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200/80 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-slate-900 text-xs truncate">{site.name}</span>
                        ...
                        <div className="space-y-1.5 text-xs">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-mono font-bold text-emerald-700">{site.riskCounts?.Low ?? 0}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-slate-500">Medium</span>
                            <span className="font-mono font-bold text-amber-700">{site.riskCounts?.Medium ?? 0}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-slate-500">High</span>
                            <span className="font-mono font-bold text-orange-700">{site.highRiskCount}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-slate-500">SIF-Precursor</span>
                            <span className="font-mono font-bold text-rose-700">{site.sifCount}</span>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 rounded-xl border border-slate-200 bg-white shadow-2xs space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-slate-600">Selected Hazard:</span>
                      <div className="w-56">
                        <Select
                          size="sm"
                ...
                {hazardLoading ? (
                  ...
                  <div className="p-6 text-center bg-white border border-rose-200 rounded-xl shadow-2xs text-xs text-rose-600">
                    {hazardError}
                  </div>
                ) : !hazardData || hazardData.siteBreakdown?.length === 0 ? (
                  <div className="p-10 text-center bg-white border border-slate-200 rounded-xl shadow-2xs text-xs text-slate-500">
                    No data recorded for {selectedHazard} across the selected facilities.
                  </div>
                ) : (
                  <>
                    {/* Site Breakdown for this Hazard (Section 5 Required Card Format) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {hazardData.siteBreakdown.map((s) => (
                        <div
                          key={s.siteId}
                          className="p-5 bg-white rounded-xl border border-slate-200 shadow-2xs space-y-3.5"
                        >
                          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                            <div>
                              ...
                              <span className="text-[10px] font-mono text-slate-400 block">{s.siteCode}</span>
                            </div>
                          ...
                          <div className="space-y-1 text-xs">
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`font-mono font-bold ${s.sifCount > 0 ? 'text-red-700' : 'text-slate-400'}`}>
                                {s.sifCount}
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-slate-500">High Risk:</span>
                              <span className={`font-mono font-bold ${s.highRiskCount > 0 ? 'text-orange-700' : 'text-slate-400'}`}>
                                {s.highRiskCount}
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                            ...
                            {s.barrierFailures && s.barrierFailures.length > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                {s.barrierFailures.map((b, idx) => (
                                  <span
                                    key={idx}
                                    className="px-2 py-0.5 rounded bg-rose-50 text-rose-800 border border-rose-200/60 text-[11px]"
                                  >
                                    {b}
                                  </span>
                                ))}
                              ...
                              <span className="text-slate-400 italic text-[11px]">No barrier breach logged</span>
                            )}
                          </div>
                      ...
                      <div className="p-5 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-3">
                        ...
                        {hazardData.recurringBarrier ? (
                          <div className="space-y-2 text-xs">
                            <div className="p-3 bg-rose-50 border border-rose-200/80 rounded-lg">
            ...
            {activeTab === 'patterns' && (
              <div className="space-y-6">
                {/* What Differs Section (Section 3.C) */}
                ...
                {/* Common Safety Patterns Section (Section 3.C) */}
                <section className="p-6 rounded-xl border border-slate-200 bg-white shadow-2xs space-y-3">
                  <div className="border-b border-slate-100 pb-2.5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                ...
                <section className="p-6 rounded-xl border border-blue-200 bg-blue-50/20 shadow-2xs space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {recommendations.map((rec, idx) => (
                      <div key={idx} className="p-3.5 bg-white border border-blue-100 rounded-lg text-xs space-y-1.5 shadow-2xs">
                        <div className="flex items-center justify-between">
